...
def test_token_with_sub_returns_current_user() -> None:
    user = decode_unverified_identity(_bearer({"sub": "alice", "email": "alice@user.io"}))
...
def test_token_with_sub_but_no_email_returns_user_with_none_email() -> None:
    user = decode_unverified_identity(_bearer({"sub": "bob"}))
    assert user is not None
    assert user.id == "bob"
...
def test_non_string_sub_is_coerced_to_string() -> None:
    user = decode_unverified_identity(_bearer({"sub": 12345}))
    assert user is not None
...
def test_token_signed_with_any_secret_is_accepted() -> None:
    ...
    user = decode_unverified_identity(token)
    assert user is not None
    assert user.id == "carol"


def test_token_without_sub_returns_none() -> None:
    assert decode_unverified_identity(_bearer({"email": "nobody@user.io"})) is None


def test_garbage_token_returns_none() -> None:
    assert decode_unverified_identity("Bearer not-a-real-jwt") is None


def test_empty_token_returns_none() -> None:
    assert decode_unverified_identity("Bearer ") is None
