...
AgentStreamEvent = TokenEvent | TraceEvent | ErrorEvent | ChartEvent | ToolCallEvent
...
`{"error": ...}` / `{"chart": ...}` / `{"tool": ...}`), so `api/v1/routers/chat.py` only
has to pattern-match this union to serialize a frame — it never needs to know about
LangGraph stream modes or payloads.
"""
