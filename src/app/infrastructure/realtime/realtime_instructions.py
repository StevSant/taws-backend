...
REALTIME_INSTRUCTIONS = (
    ...
    "- Use your tools silently. NEVER say 'the tools I need', name your tools, or narrate that "
    "you are about to call one — just call it. Only exception: right before the slower "
    "generate_signal, say one brief line like 'let me pull that' so the pause makes sense.\n\n"
    ...
    "render_comparison_chart, render_macro_chart, render_drawdown_chart, render_distribution_chart, "
    "and render_sentiment_gauge display interactive visuals on the user's screen — a request to "
    "show, plot, draw, or chart something means calling the matching render tool, then a one-line "
    "summary (never claim a chart can't be shown). get_watchlist and get_notes return the "
    ...
    "call get_news for that symbol (and closely related ones) and give your read and recommendation "
    "from what you find — only say you lack data after you've actually tried to pull more news."
)
