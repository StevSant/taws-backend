...
def parse_start_command(update_payload: dict[str, Any]) -> StartCommand | None:
    ...
    Returns `None` for anything that isn't a `/start <token>` text message: Telegram
    webhooks deliver many other update kinds (edited messages, callback queries, chat
    member updates, ...) that this integration deliberately ignores.
    """
    ...
    if not isinstance(text, str) or not text.strip().startswith(_START_PREFIX):
        return None

    token = text.strip()[len(_START_PREFIX) :].strip()
    if not token:
        return None
